...
function displayItems(){
   ...
   const html = items.map(function(item){
      ...
      <input type="checkbox" checked=${item.complete}?${item.complete}:false value="${item.id}">
...
list.addEventListener('click',function(e){
    ...
    if(e.target.matches('input[type="checked"]')){
...
function markAsComplete(id){
    ...
    const itemRef= items.find(function(element){
        if(element.id === id){
            return element;
        }
    })
    to toggle check and uncheck
